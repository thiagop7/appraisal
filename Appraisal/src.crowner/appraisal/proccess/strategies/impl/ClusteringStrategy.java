...
import appraisal.proccess.plans.impl.ClusteringPlan;
...
public class ClusteringStrategy implements Strategy 
{
	// Logger
	private static final Log LOGGER = LogFactory.getLog(ClusteringStrategy.class);
	...
	public ClusteringStrategy(boolean pso,	boolean kmeans,	boolean knn, boolean bkprop,boolean avg)
	{
		this.pso = pso;
		this.kmeans = kmeans;
		this.knn = knn;
	...
	{
		LOGGER.info("### ESTRATÉGIA DE CLUSTERIZAÇÃO E REGRESSÃO ###");
		...
		for (ClusteringStage clusteringStage : clusteringStages) 
		{
			...
			{
				ClusteringPlan plan = new ClusteringPlan(clusteringStage,regressionStage);
				plans.add(plan);
			}
		}
		...
		for (Plan plan : plans) 
		{
			...
			ResultWriter.writeCrudeResults(new StrategyResult("clustering.regression",planResults) );
		...
		StrategyResult strategyResult = new StrategyResult("clustering.regression",planResults);
