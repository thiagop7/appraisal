...
import appraisal.proccess.plans.impl.SelectionClusteringPlan;
...
public class SelectionClusteringStrategy implements Strategy 
{
	// Logger
	private static final Log LOGGER = LogFactory.getLog(SelectionClusteringStrategy.class);
	...
	public SelectionClusteringStrategy(boolean pso,	boolean kmeans,	boolean knn, boolean bkprop, boolean avg)
	{
		this.pso = pso;
		this.kmeans = kmeans;
		this.knn = knn;
	...
	{
		LOGGER.info("### ESTRATÉGIA DE SELEÇÃO, CLUSTERIZAÇÃO E REGRESSÃO ###");
		...
		for (SelectionStage selectionStage : selectionStages) 
		{
			for (ClusteringStage clusteringStage : clusteringStages)
			{
				...
				{
					SelectionClusteringPlan plan = new SelectionClusteringPlan(selectionStage,clusteringStage,regressionStage);
					plans.add(plan);
				}
			}
		}
		...
		for (Plan plan : plans) 
		{
			...
			ResultWriter.writeCrudeResults(new StrategyResult("selection.clustering.regression",planResults) );
		...
		StrategyResult strategyResult = new StrategyResult("selection.clustering.regression",planResults);
